		...
		if (!g_cl.m_lag && g_csgo.m_globals->m_curtime >= g_cl.m_body_pred && (stand || air)) {
			...
			if (stand) {
				...
				case 3:
					...
					g_cl.m_cmd->m_view_angles.y += 127.f;
					break;
				...
				case 4:
					...
					negative ? g_cl.m_cmd->m_view_angles.y += 120.f : g_cl.m_cmd->m_view_angles.y -= 120.f;
					negative = !negative;
					break;
				...
				case 7:
					...
					g_cl.m_cmd->m_view_angles.y += 133.f;
					break;
					g_cl.m_cmd->m_view_angles.y += 133.f;
					break;
...
void HVH::DoFakeAntiAim() {
	...
	*g_cl.m_packet = true;
	...
	case 1:
		...
		g_cl.m_cmd->m_view_angles.y = m_direction + 127.f;

		// apply 50 degree jitter.
		g_cl.m_cmd->m_view_angles.y += g_csgo.RandomFloat(-100.f, 100.f);
		break;
	...
	case 5:
		...
		// spin bot.
	case 7:
		g_cl.m_cmd->m_view_angles.y = m_direction + 90.f + std::fmod(g_csgo.m_globals->m_curtime * 140.f, 140.f);
		break;
