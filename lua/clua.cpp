...
void c_lua::refresh_scripts() {
	...
	//*for (auto& entry : std::filesystem::directory_iterator("C:\\Burgerware.xyz\\lua")) {
	//f (entry.path().extension() == (".lua")) {
			//to path = entry.path();
			//to filename = path.filename().string();

			//ol didPut = false;
			//r (int i = 0; i < oldScripts.size(); i++) {
			//f (filename == oldScripts.at(i)) {
				//his->loaded.push_back(oldLoaded.at(i));
				//idPut = true;

			// (!didPut)
			//his->loaded.push_back(false);

		//his->pathes.push_back(path);
	///his->scripts.push_back(filename);
		//
	//
}
