...
void Hooks::FrameStageNotify(Stage_t stage) {
	...
	{
		for (auto hk : g_lua_hook.getHooks("net_update_start"))
			hk.func();
	}
	if (stage == FRAME_NET_UPDATE_END)
	{
		for (auto hk : g_lua_hook.getHooks("net_update_end"))
			hk.func();
		//static auto rain = false;

		//if (rain != g_hooks.m[XOR("brightnessAdjustment_sel")][0] || g_ctx.globals.should_update_weather)
		//{
		//	rain = g_hooks.m[XOR("brightnessAdjustment_sel")][0];
