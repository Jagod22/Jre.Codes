	...
	for( auto i : g_lua.menu_items[ tab ][ container ] ) {
		...
		switch( type ) {
			case MENUITEM_CHECKBOX:
				if( ImGui::Checkbox( i.label.c_str( ), &g_hooks.b[ i.key ] ) ) {
					if( i.callback != sol::nil )
						i.callback( g_hooks.b[ i.key ] );
				}

				//dmt( i.key );
				break;
			case MENUITEM_SLIDERINT:
				if( ImGui::SliderInt( i.label.c_str( ), &g_hooks.i[ i.key ], i.i_min, i.i_max, i.format.c_str( ) ) ) {
					if( i.callback != sol::nil )
						i.callback( g_hooks.i[ i.key ] );
				}

				//dmt( i.key );
				break;
			case MENUITEM_SLIDERFLOAT:
				if( ImGui::SliderFloat( i.label.c_str( ), &g_hooks.f[ i.key ], i.f_min, i.f_max, i.format.c_str( ) ) ) {
					if( i.callback != sol::nil )
						i.callback( g_hooks.f[ i.key ] );
				}

				//dmt( i.key );
				break;
			case MENUITEM_KEYBIND:
				if( ImGui::Keybind( i.label.c_str( ), &g_hooks.i[ i.key ], i.allow_style_change ? &g_hooks.i[ i.key + XOR( "style" ) ] : NULL ) ) {
					if( i.callback != sol::nil )
						i.callback( g_hooks.i[ i.key ], g_hooks.i[ i.key + XOR( "style" ) ] );
				}
			...
			case MENUITEM_SINGLESELECT:
				if( ImGui::SingleSelect( i.label.c_str( ), &g_hooks.i[ i.key ], i.items ) ) {
					if( i.callback != sol::nil )
						i.callback( g_hooks.i[ i.key ] );
				}
			...
			case MENUITEM_MULTISELECT:
				if( ImGui::MultiSelect( i.label.c_str( ), &g_hooks.m[ i.key ], i.items ) ) {
					if( i.callback != sol::nil )
						i.callback( g_hooks.m[ i.key ] );
				}

				//dmt( i.key );
				break;
			case MENUITEM_COLORPICKER:
				if( ImGui::ColorEdit4( i.label.c_str( ), g_hooks.c[ i.key ] ) ) {
					if( i.callback != sol::nil )
						i.callback( g_hooks.c[ i.key ][ 0 ] * 255, g_hooks.c[ i.key ][ 1 ] * 255, g_hooks.c[ i.key ][ 2 ] * 255, g_hooks.c[ i.key ][ 3 ] * 255 );
				}
...
void Hooks::save( ) {
	...
	sprintf( file_path, XOR( "C:/jre.codes/cfg/%s.idb" ), presets.at( i[ XOR( "_preset" ) ] ) );
...
void Hooks::load_keys( ) {
	for( int k = 0; k < presets.size( ); k++ ) {
		...
		i[ buffer ] = GetPrivateProfileIntA( XOR( "k" ), buffer, 0, XOR( "C:/jre.codes/cfg/eys.idb" ) );
	}
}
...
void Hooks::save_keys( ) {
	for( int k = 0; k < presets.size( ); k++ ) {
		...
		WritePrivateProfileStringA( XOR( "k" ), buffer, value, XOR( "C:/jre.codes/cfg/keys.idb" ) );
	}
}
...
void Hooks::Aimbot( ) {
	...
	ImGui::Columns( 2, NULL, false );
	{
		...
		InsertGroupBoxLeft( XOR( "Aimbot" ), 287.f );
		{
			...
			InsertCombo( XOR( "Adaptive weapons" ), &i[ XOR( "weapon_cnofig" ) ], weapon_mode );
			...
			InsertCheckbox( XOR( "rapidfire" ), b[ XOR( "rapidfire" ) ] );
			InsertStyleKeyBind( XOR( "##Rapid fire key" ), i[ XOR( "rapidfire_key" ) ], &i[ XOR( "rapidfire_keystyle" ) ] );
		...
		InsertGroupBoxLeft( XOR( "Fake lag" ), 150.f );
		{
			...
			draw_lua_items( "ragebot", "Fakelag" );
	...
	ImGui::NextColumn( );
	{
		...
		InsertGroupBoxRight( XOR( "Anti-aimbot" ), 247.f );
		{
			...
			draw_lua_items( "ragebot", "AntiAims" );
...
void Hooks::Misc( ) {
	...
	ImGui::Columns( 2, NULL, false );
	{
		...
		InsertGroupBoxLeft( XOR( "Miscellaneous" ), 455.f );
		{
			...
			if( !luaconfigs ) {
				...
				draw_lua_items( "Miscellaneous", "Miscellaneous" );

				ImGui::PopItemWidth( ); ImGui::CustomSpacing( 1.f );
			}
			else {
				for( auto z : g_hooks.luaMenuItems ) {
					if( g_lua.loaded.at( z.first ) ) {
						for( auto i : z.second ) {
							switch( ( luaMenuItem::luamenuTypes )i.type ) {
								case luaMenuItem::luamenuTypes::LUAMENU_CHECKBOX:
								{
									InsertCheckbox( i.name.c_str( ), g_hooks.b[ i.tag ] /**(bool*)i.value*/ );

									//g_hooks.b[name] = 0;
									//newItem.value = &g_hooks.b[name];
								}
								break;
								//			

								case luaMenuItem::luamenuTypes::LUAMENU_SLIDER:
								{
									InsertSlider( i.name.c_str( ), g_hooks.f[ i.tag ], i.slidermin, i.slidermax, "%1.f" );
									//InsertSlider(XOR("Body hitbox scale##0"), f[XOR("body_scale")], 1.f, 100.f, XOR("%1.f%%"));
								...
								case luaMenuItem::luamenuTypes::LUAMENU_COMBO:
								{
									std::vector<const char*> bitching;

									for( auto& str : i.comboitems )
										bitching.push_back( str.c_str( ) );

									InsertCombo( i.name.c_str( ), &g_hooks.i[ i.tag ], bitching );
								}
								break;
	...
	ImGui::NextColumn( );
	{
		...
		InsertGroupBoxRight( XOR( "Other" ), 156.f );
		{
			...
			style->ItemSpacing = ImVec2( 0, 0 );
			style->WindowPadding = ImVec2( 6, 6 );
			draw_lua_items( "Miscellaneous", "Other" );
		...
		InsertGroupBoxRight( XOR( "Settings" ), 281.f );
		{
			...
			if( !lua ) {
				...
				if( ImGui::Button( XOR( "Load" ) ) ) {
					...
					if( !std::filesystem::exists( XOR( "C:/jre.codes" ) ) )
						std::filesystem::create_directories( XOR( "C:/jre.codes" ) );

					if( !std::filesystem::exists( XOR( "C:/jre.codes/cfg" ) ) )
						std::filesystem::create_directories( XOR( "C:/jre.codes/cfg" ) );
				...
				if( ImGui::Button( XOR( "Open settings folder" ) ) ) {
					ShellExecuteA( 0, XOR( "open" ), XOR( "C:/jre.codes" ), NULL, NULL, SW_NORMAL );
					if( !std::filesystem::exists( XOR( "C:/jre.codes" ) ) )
						std::filesystem::create_directories( XOR( "C:/jre.codes" ) );
				...
				if( ImGui::Button( XOR( "Refresh scripts" ) ) )
					g_lua.refresh_scripts( );
				ImGui::Spacing( ); ImGui::NewLine( ); ImGui::SameLine( 42.f ); ImGui::ListBoxHeader( "##urnn", ImVec2( 0, 160 ) );
				{
...
void SpectatorList( ) {
	...
	ImGui::Begin( XOR( "##SPECTATORS" ), NULL, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBackground );
	{
		...
		{//eyes
			ImGui::PushFont( font );//time
			draw->AddRectFilled( ImVec2( p.x, p.y + 20 ), ImVec2( p.x + s.x, p.y + ( ( spectators.size( ) + 1 ) * 14 ) + 10 ), ImColor( 30, 31, 32, 140 ), 4, ImDrawCornerFlags_Bot );
			for( size_t i{ }; i < spectators.size( ); ++i ) {
				...
				const std::string& observe = spectators[ i ].ObserverMode;
				{
					...
					draw->AddText( ImVec2( p.x + 15, p.y + 22 + ( i * 14 ) ), ImColor( 220, 220, 220 ), name.c_str( ) );
...
void KeyBind( ) {
	...
	if( g_hooks.auto_check( "rapidfire_key" ) && g_hooks.b[ XOR( "rapidfire" ) ] ) {
		KeyBind_t KeybindList;
		KeybindList.m_name = XOR( "rapidfire" );
		KeybindList.m_mode = check( XOR( "rapidfire_key" ) );
		...
		{//bind
			...
			for( size_t i{ }; i < keybinds.size( ); ++i ) {
				...
				const std::string& observe = keybinds[ i ].m_mode;
				{
					...
					draw->AddText( ImVec2( p.x + 15 + 1, p.y + 22 + ( i * 14 ) + 1 ), ImColor( 0, 0, 0 ), name.c_str( ) );
					draw->AddText( ImVec2( p.x + ( s.x - 5 - ImGui::CalcTextSize( observe.c_str( ) ).x ) + 1, p.y + 22 + ( i * 14 ) + 1 ), ImColor( 0, 0, 0 ), observe.c_str( ) );
...
void Hooks::imgui_menu( ) {
	...
	ImGui::BeginMenuBackground( XOR( "jre.codes" ), &g_hooks.Is_Open, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoNavFocus | ImGuiWindowFlags_NoNavInputs | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoTitleBar );
	{
		...
		ImGui::BeginChild( XOR( "Menu Contents" ), ImVec2( 628.f, 548.f ), false );
		{
			...
			ImGui::BeginChild( XOR( "Tab Contents" ), ImVec2( 572.f, 542.f ), false );
			{
				...
				style->Colors[ ImGuiCol_Border ] = ImColor( 10, 10, 10, m_alpha );

			} ImGui::EndChild( );

			style->ItemSpacing = ImVec2( 4.f, 4.f );
			style->Colors[ ImGuiCol_ChildBg ] = ImColor( 17, 17, 17, m_alpha );
...
void initialize( IDirect3DDevice9* pDevice ) {
	...
	menuFont = io.Fonts->AddFontFromMemoryCompressedTTF( verdana_compressed_data, verdana_compressed_size, 11, &font_config, ranges );
	controlFont = io.Fonts->AddFontFromMemoryCompressedTTF( comboarrow_compressed_data, comboarrow_compressed_size, 12, &font_config, ranges );
	boldMenuFont = io.Fonts->AddFontFromMemoryCompressedTTF( verdanab_compressed_data, sizeof verdanab_compressed_data, 11.5, &font_config, ranges );
	tabFont = io.Fonts->AddFontFromMemoryTTF( &NanoX1, sizeof NanoX1, 50.0f, &font_config, ranges );
	keybinds = io.Fonts->AddFontFromMemoryTTF( keybinds_font, 25600, 10.f, &font_config, ranges );
	weaponzFont = io.Fonts->AddFontFromMemoryTTF( &WeaponzFont, sizeof WeaponzFont, 27.0f, &font_config, ranges );
	weapon_icons = io.Fonts->AddFontFromMemoryTTF( &icomoon, sizeof icomoon, 38.0f, &font_config, ranges );
	font = io.Fonts->AddFontFromMemoryTTF( &MuseoSansCyrl700, sizeof MuseoSansCyrl700, 14.0f, &font_config, ranges );
