...
void Client::DrawHUD( ) {
	...
	const auto col_accent = Color( g_hooks.c[ XOR( "menu_color" ) ][ 0 ] * 255.f, g_hooks.c[ XOR( "menu_color" ) ][ 1 ] * 255.f, g_hooks.c[ XOR( "menu_color" ) ][ 2 ] * 255.f );
	...
	std::string text = tfm::format( XOR( "jre.codes | %s | delay: %ims | tick: %i | fps: %i | %s" ), g_cl.m_user, ms, rate, fps, time.str( ).data( ) );
	...
	// text.
	render::menu_shade.string( m_width - 14, 14, { 255, 255, 255, 255 }, tfm::format( XOR( " | %s | delay: %ims | tick: %i | fps: %i | %s" ), g_cl.m_user, ms, rate, fps, time.str( ).data( ) ), render::ALIGN_RIGHT );
	render::menu_shade.string( m_width - 14 - render::menu_shade.size( tfm::format( XOR( " | %s | delay: %ims | tick: %i | fps: %i | %s" ), g_cl.m_user, ms, rate, fps, time.str( ).data( ) ) ).m_width, 14, col_accent, tfm::format( XOR( ".codes" ) ), render::ALIGN_RIGHT );
	render::menu_shade.string( m_width - 14 - render::menu_shade.size( tfm::format( XOR( ".codes | %s | delay: %ims | tick: %i | fps: %i | %s" ), g_cl.m_user, ms, rate, fps, time.str( ).data( ) ) ).m_width, 14, { 255, 255, 255, 255 }, tfm::format( XOR( "jre" ) ), render::ALIGN_RIGHT );
}
...
void Client::StartMove( CUserCmd* cmd ) {
	...
	for( auto hk : g_lua_hook.getHooks( XOR( "on_createmove" ) ) )
		hk.func( cmd, *m_packet );
...
void Client::DoMove( ) {
	...
	for( auto hk : g_lua_hook.getHooks( XOR( "inprediction" ) ) )
		hk.func( m_cmd, *m_packet );
...
void Client::OnTick( CUserCmd* cmd ) {
	...
	for( auto hk : g_lua_hook.getHooks( XOR( "afterprediction" ) ) )
		hk.func( m_cmd, *m_packet );
...
void Client::print( const std::string text, ... ) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf( colors::yellowgreen, XOR( "[jre.codes] " ) );
...
void Client::ClanTag( ) {
	...
	auto SetClanTag = [ & ]( std::string tag ) -> void {
		...
		SetClanTagFn( tag.c_str( ), XOR( "gamesense" ) );
	};

	std::string szClanTag = XOR( "      gamesense     " );
	...
	// gamesense.
		...
		// are we in a new frame?
		if( iPrevFrame != ( int )( g_csgo.m_globals->m_curtime * 2.9f ) % 20 ) {
			switch( int( g_csgo.m_globals->m_curtime * 2.9f ) % 20 ) {
				case 0: SetClanTag( XOR( "          " ) ); break;
				case 1: SetClanTag( XOR( "        j" ) ); break;
				case 2: SetClanTag( XOR( "       jr" ) ); break;
				case 3: SetClanTag( XOR( "      jre" ) ); break;
				case 4: SetClanTag( XOR( "     jre." ) ); break;
				case 5: SetClanTag( XOR( "    jre.c" ) ); break;
				case 6: SetClanTag( XOR( "    jre.co" ) ); break;
				case 7: SetClanTag( XOR( "  jre.cod" ) ); break;
				case 8: SetClanTag( XOR( " jre.codes" ) ); break;
				case 9: SetClanTag( XOR( "jre.codes " ) ); break;
				case 10:SetClanTag( XOR( "re.codes  " ) ); break;
				case 11:SetClanTag( XOR( "e.codes   " ) ); break;
				case 12:SetClanTag( XOR( ".codes    " ) ); break;
				case 13:SetClanTag( XOR( "codes    " ) ); break;
				case 14:SetClanTag( XOR( "odes      " ) ); break;
				case 15:SetClanTag( XOR( "des       " ) ); break;
				case 16:SetClanTag( XOR( "es       " ) ); break;
				case 17:SetClanTag( XOR( "s         " ) ); break;
				case 18:SetClanTag(XOR("s         ")); break;
			}

			// set current/last frame.
			iPrevFrame = ( int )( g_csgo.m_globals->m_curtime * 2.9f ) % 24;
		}
	...
	// skeet.cc.
		...
		std::string szClanTag2 = XOR( "bluhgang" );
