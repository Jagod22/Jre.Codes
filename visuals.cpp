...
void Visuals::DrawPlayer( Player* player ) {
	...
	// draw flags.
	{
		...
		if( g_hooks.b[ XOR( "flags" ) ] && g_cl.m_processing ) {
			if( g_resolver.resolver_state[ player->index( ) ] != XOR( "RESOLVE_BODY" ) && g_resolver.resolver_state[ player->index( ) ] != XOR( "RESOLVE_WALK" ) &&
				g_resolver.resolver_state[ player->index( ) ] != XOR( "RESOLVE_AIR" ) && g_resolver.resolver_state[ player->index( ) ] != XOR( "RESOLVE_NONE" ) &&
				g_resolver.resolver_state[ player->index( ) ] != XOR( "RESOLVE_STOPPED_MOVING" ) )
				flags.push_back( { XOR( "FAKE" ), { 220, 220, 220, low_alpha } } );
		}
